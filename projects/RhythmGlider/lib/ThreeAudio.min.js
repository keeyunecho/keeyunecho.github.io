...
function FourierTransform(a,c){this.bufferSize=a;this.sampleRate=c;this.bandwidth=2/a*c/2;this.spectrum=new Float32Array(a/2);this.real=new Float32Array(a);this.imag=new Float32Array(a);this.peak=this.peakBand=0;this.getBandFrequency=function(a){return this.bandwidth*a+this.bandwidth/2};this.calculateSpectrum=function(){for(var c=this.spectrum,e=this.real,d=this.imag,n=2/this.bufferSize,m=Math.sqrt,h,k,s=0,p=a/2;s<p;s++)h=e[s],k=d[s],h=n*m(h*h+k*k),h>this.peak&&(this.peakBand=s,this.peak=h),c[s]=
h}}function DFT(a,c){FourierTransform.call(this,a,c);var f=a/2*a,e=2*Math.PI;this.sinTable=new Float32Array(f);this.cosTable=new Float32Array(f);for(var d=0;d<f;d++)this.sinTable[d]=Math.sin(d*e/a),this.cosTable[d]=Math.cos(d*e/a)}DFT.prototype.forward=function(a){for(var c=this.real,f=this.imag,e,d,n=0;n<this.bufferSize/2;n++){for(var m=d=e=0;m<a.length;m++)e+=this.cosTable[n*m]*a[m],d+=this.sinTable[n*m]*a[m];c[n]=e;f[n]=d}return this.calculateSpectrum()};
...
FFT.prototype.forward=function(a){var c=this.bufferSize,f=this.cosTable,e=this.sinTable,d=this.reverseTable,n=this.real,m=this.imag,h=Math.floor(Math.log(c)/Math.LN2);if(Math.pow(2,h)!==c)throw"Invalid buffer size, must be a power of 2.";if(c!==a.length)throw"Supplied buffer is not the same size as defined FFT. FFT Size: "+c+" Buffer Size: "+a.length;var h=1,k,s,p,q,t,g;for(g=0;g<c;g++)n[g]=a[d[g]],m[g]=0;for(;h<c;){a=f[h];d=e[h];k=1;for(var w=s=0;w<h;w++){for(g=w;g<c;)p=g+h,q=k*n[p]-s*m[p],t=k*m[p]+
s*n[p],n[p]=n[g]-q,m[p]=m[g]-t,n[g]+=q,m[g]+=t,g+=h<<1;g=k;k=g*a-s*d;s=g*d+s*a}h<<=1}return this.calculateSpectrum()};
FFT.prototype.inverse=function(a,c){var f=this.bufferSize,e=this.cosTable,d=this.sinTable,n=this.reverseTable;a=a||this.real;c=c||this.imag;var m=1,h,k,s,p,q,t,g;for(g=0;g<f;g++)c[g]*=-1;h=new Float32Array(f);k=new Float32Array(f);for(g=0;g<a.length;g++)h[g]=a[n[g]],k[g]=c[n[g]];a=h;for(c=k;m<f;){n=e[m];h=d[m];k=1;for(var w=s=0;w<m;w++){for(g=w;g<f;)p=g+m,q=k*a[p]-s*c[p],t=k*c[p]+s*a[p],a[p]=a[g]-q,c[p]=c[g]-t,a[g]+=q,c[g]+=t,g+=m<<1;g=k;k=g*n-s*h;s=g*h+s*n}m<<=1}e=new Float32Array(f);for(g=0;g<f;g++)e[g]=
a[g]/f;return e};
function RFFT(a,c){FourierTransform.call(this,a,c);this.trans=new Float32Array(a);this.reverseTable=new Uint32Array(a);this.reverseBinPermute=function(a,c){var d=this.bufferSize,n=d>>>1,d=d-1,m=1,h=0,k;a[0]=c[0];do{h+=n;a[m]=c[h];a[h]=c[m];m++;for(k=n<<1;k>>=1,!((h^=k)&k););h>=m&&(a[m]=c[h],a[h]=c[m],a[d-m]=c[d-h],a[d-h]=c[d-m]);m++}while(m<n);a[d]=c[d]};this.generateReverseTable=function(){var a=this.bufferSize,c=a>>>1,a=a-1,d=1,n=0,m;this.reverseTable[0]=0;do{n+=c;this.reverseTable[d]=n;this.reverseTable[n]=
d;d++;for(m=c<<1;m>>=1,!((n^=m)&m););n>=d&&(this.reverseTable[d]=n,this.reverseTable[n]=d,this.reverseTable[a-d]=a-n,this.reverseTable[a-n]=a-d);d++}while(d<c);this.reverseTable[a]=a};this.generateReverseTable()}
RFFT.prototype.forward=function(a){var c=this.bufferSize,f=this.spectrum,e=this.trans,d=2*Math.PI,n=Math.sqrt,m=c>>>1,h=2/c,k,s,p,q,t,g,w,r,y,u,x,z,E,F,H,B,C,D,I,J,K;this.reverseBinPermute(e,a);q=0;for(var A=4;q<c;A*=4){for(var v=q;v<c;v+=A)B=e[v]-e[v+1],e[v]+=e[v+1],e[v+1]=B;q=2*(A-1)}a=2;for(p=c>>>1;p>>>=1;){q=0;a<<=1;A=a<<1;k=a>>>2;s=a>>>3;do{if(1!==k)for(v=q;v<c;v+=A)r=v,y=r+k,u=y+k,x=u+k,q=e[u]+e[x],e[x]-=e[u],e[u]=e[r]-q,e[r]+=q,r+=s,y+=s,u+=s,x+=s,q=e[u]+e[x],t=e[u]-e[x],q=-q*Math.SQRT1_2,
t*=Math.SQRT1_2,B=e[y],e[x]=q+B,e[u]=q-B,e[y]=e[r]-t,e[r]+=t;else for(v=q;v<c;v+=A)r=v,y=r+k,u=y+k,x=u+k,q=e[u]+e[x],e[x]-=e[u],e[u]=e[r]-q,e[r]+=q;q=(A<<1)-a;A<<=2}while(q<c);K=d/a;for(var G=1;G<s;G++){C=G*K;D=Math.sin(C);C=Math.cos(C);I=4*C*(C*C-0.75);J=4*D*(0.75-D*D);q=0;A=a<<1;do{for(v=q;v<c;v+=A)r=v+G,y=r+k,u=y+k,x=u+k,z=v+k-G,E=z+k,F=E+k,H=F+k,t=e[F]*C-e[u]*D,q=e[F]*D+e[u]*C,w=e[H]*I-e[x]*J,g=e[H]*J+e[x]*I,B=t-w,t+=w,w=B,e[H]=t+e[E],e[u]=t-e[E],B=g-q,q+=g,g=B,e[x]=g+e[y],e[F]=g-e[y],e[E]=e[r]-
q,e[r]+=q,e[y]=w+e[z],e[z]-=w;q=(A<<1)-a;A<<=2}while(q<c)}}for(;--m;)d=e[m],a=e[c-m-1],d=h*n(d*d+a*a),d>this.peak&&(this.peakBand=m,this.peak=d),f[m]=d;f[0]=h*e[0];return f};(function(a){for(var c in a)if(!window[c])throw"Error: ThreeAudio requires "+a[c];})({THREE:"Three.js",MicroEvent:"MicroEvent.js"});window.ThreeAudio=window.ThreeAudio||{};ThreeAudio.getShader=function(a){var c=document.getElementById(a);return c&&(c.innerText||c.textContent)||a};window._=window._||{};
...
this.element.addEventListener("canplay",function(){this.initAnalyzer()}.bind(this))},initAnalyzer:function(){var a=this.context;this.source=a.createMediaElementSource(this.element);this.source.connect(this.audible);this.analyser=a.createAnalyser();var c=this.analyser.fftSize=this.fftSize,f=this.filters;_.each({bass:{type:0,frequency:160,Q:1.2,gain:2},mid:{type:2,frequency:500,Q:1.2,gain:4},treble:{type:1,frequency:2E3,Q:1.2,gain:3}},function(d,e){var h=a.createBiquadFilter();h.key=e;h.type=d.type;
h.frequency.value=d.frequency;h.Q.value=d.Q;h.analyser=a.createAnalyser();h.analyser.fftSize=c;h.delayNode=a.createDelayNode();h.delayNode.delayTime.value=0;h.gainNode=a.createGainNode();h.gainNode.gain.value=d.gain;f[e]=h});this.delay=a.createDelayNode();this.processingDelay=2*this.fftSize/a.sampleRate;this.delay.delayTime.value=this.processingDelay;this.audible.connect(this.analyser);this.inaudible.connect(this.analyser);this.audible.connect(this.delay);this.delay.connect(a.destination);var e=this.audible,
...
ThreeAudio.Textures.prototype={init:function(){var a=this.renderer,c=a.getContext(),f=this.textures,e=this.data,d=this.history,n,m,h;_.each(["freq","time"],function(k){f[k]&&(c.deleteTexture(f[k]),a.info.memory.textures--);n=f[k]=c.createTexture();a.info.memory.textures++;c.bindTexture(c.TEXTURE_2D,n);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_S,c.CLAMP_TO_EDGE);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_WRAP_T,c.REPEAT);c.texParameteri(c.TEXTURE_2D,c.TEXTURE_MIN_FILTER,c.LINEAR);c.texParameteri(c.TEXTURE_2D,
c.TEXTURE_MAG_FILTER,c.LINEAR);m=e[k];h=new Uint8Array(m.length*d);if("time"==k)for(k=0;k<m.length*d;++k)h[k]=128;c.pixelStorei(c.UNPACK_ALIGNMENT,1);c.texImage2D(c.TEXTURE_2D,0,c.ALPHA,m.length,d,0,c.ALPHA,c.UNSIGNED_BYTE,h)})},update:function(){var a=this.renderer.getContext(),c=this.textures,f=this.source,e=this.history,d=this.timeIndex,n=f.data;f.update();_.each(["freq","time"],function(e){a.bindTexture(a.TEXTURE_2D,c[e]);a.pixelStorei(a.UNPACK_ALIGNMENT,1);a.texSubImage2D(a.TEXTURE_2D,0,0,d,
...
ThreeAudio.GridGeometry=function(a,c,f,e,d){var n=a.source;e=(e||n.samples)-1;d=Math.min(a.history,d||a.history)-1;var m=d/a.history,h=0.5/(n.samples-1);offsetV=0.5/a.history;a=new THREE.PlaneGeometry(c,f,e,d);_.each(a.faceVertexUvs[0],function(a){_.each(a,function(a){a.u+=h;a.v=1-a.v*m+offsetV})});return a};ThreeAudio.LevelDetect=function(a){this.data=a;this.levels=[[0,0,0,0]];this.smooth=[0,0,0,0];this.change=[0,0,0,0];a.levels={direct:this.levels[0],smooth:this.smooth,change:this.change}};
ThreeAudio.LevelDetect.prototype.analyse=function(){for(var a=this.data,c=this.levels,f=this.smooth,e=this.change,d=[0,0,0,0],n=[a.time,a.filter.bass,a.filter.mid,a.filter.treble],a=0;4>a;++a){var m=d,h=a,k;k=n[a];for(var s=k.length,p=0,q=0;q<s;++q)var t=(k[q]-128)/128,p=p+t*t;k=Math.sqrt(p/s);m[h]=k}c.unshift(d);7<c.length&&c.pop();n=[1,2,1];m=4;h=Math.min(c.length,n.length);for(k=0;4>k;++k){for(a=d=0;a<h;++a)d+=(c[a]&&c[a][k]||0)*n[a];f[k]=d/m}n=[1,3,2,-2,-3,-1];m=1;h=Math.min(c.length,n.length);
for(k=0;4>k;++k){for(a=d=0;a<h;++a)d+=(c[a]&&c[a][k]||0)*n[a];e[k]+=0.5*(d/m-e[k])}this.data.levels.direct=this.levels[0]};var __taDebug=!1;
...
"absolute";this.t.style.zIndex=20;this.t.style.marginTop="350px";this.c.style.top=0;document.body.appendChild(this.c);document.body.appendChild(this.t)},bpmToOffset:function(a){return this.n*a/3600},offsetToBPM:function(a){return 3600*a/this.n},analyse:function(){function a(a){var c=0,d=1.5*a.offset;_.each(k,function(e){if(e!=a&&!(e.offset<d)){var f=e.offset/a.offset,g=Math.max(0,1-4*Math.abs(f-Math.round(f)));w=e.strength*e.permanence*f;c+=g*w}});return c}var c=this.data,f=c.levels,e=this.buffer,
d=this.fft,n=this.n,m=this.history,h=this.histogram,k=this.histogramSorted,s=this;c.beat.missed=!1;c.beat.maybe=!1;c.beat.is=!1;c.beat.predicted=!1;c.beat.locked=!1;c.beat.adjusted=!1;this.beat?(c.beat.confidence=this.beat.confidence,c.beat.permanence=this.beat.permanence,c.beat.bpm=this.beat.bpm):(c.beat.confidence=0,c.beat.permanence=0,c.beat.bpm="");var p=f.direct[0];this.background+=0.2*(p-this.background);this.energy+=0.4*(p-this.energy);this.signal=p=(this.energy-this.background)/(1-this.background)*
3;f=this.maybe;maybe=p;this.maxMaybe=Math.max(0.99*this.maxMaybe,maybe);this.maybe=maybe=maybe/Math.max(0.2,this.maxMaybe)-0.7;this.sample=p;for(m.unshift(p);m.length>n;)m.pop();e.set(m);d.forward(e);for(var q=this.spectrum=d.real,p=d.real,e=d.imag,d=0;d<n;++d)q[d]=p[d]*p[d]+e[d]*e[d];e=0;for(d=2;d<n/8;++d)e=Math.max(e,q[d]);for(var m={},t=e/16,d=this.fMin+1;d<this.fMax;++d)if(p=q[d],p>t){var g=Math.max(q[d-1],q[d+1]);if(p>g&&(g=Math.min(q[d-1],q[d+1]),0.5<Math.abs(p-g)/p)){var w=Math.sqrt(p/e);m[d]=
w}}_.each(k,function(a,c){a.permanence*=0.99;decay=Math.min(1,Math.log(1+a.permanence)/Math.log(10));a.strength*=decay;a.active=!1});_.each(m,function(a,c){c=+c;var d=q[c-1],e=q[c+1],f=d+e-2*q[c];b=(e-d)/2;fraction=-b/f;d=h[c];d||((d=h[c+1])?(delete h[c+1],d.index=c,d.fraction+=1,h[c]=d):(d=h[c-1])?(delete h[c-1],d.index=c,d.fraction-=1,h[c]=d):(d=h[c]={index:c,fraction:fraction,offset:0,strength:0,permanence:0,score:0,window:0},k.push(d)));d.fraction+=0.1*(fraction-d.fraction);d.strength=a;d.permanence+=
0.01*d.strength;d.offset=d.index+d.fraction;d.bpm=s.offsetToBPM(d.offset);d.window=n/d.offset;d.active=!0});k.sort(function(a,c){return c.permanence-a.permanence});for(var r;(r=k[k.length-1])&&0.01>r.strength;)k.splice(k.length-1,1),delete h[r.index];var y=this.fMin,u=0,x=0,z=null;_.each(k,function(c){var d=0;if(!(c.offset<y)){var e=a(c);e>d&&(d=e);d+=c.strength*c.permanence;c.score=d;d>u?(z=c,x=u,u=d):d>x&&(x=d)}});z&&(z.score=u?1-x/u:0,z.confidence=Math.min(1,z.score+z.permanence),this.beat?this.beat.confidence<
...
r=0;l=f.length;for(d=0;d<l;++d)r+=f[d],p+=f[d]*f[d];r/=l;p/=l;this.mean=r;this.stddev=Math.sqrt(p-r*r)}this.jitter=(this.stddev+0.5*this.missed)*(1+this.missed);this.decay+=0.4*(2.5*+c.beat.is-this.decay);c.beat.was+=0.4*(2.5*this.decay-c.beat.was);this.debounceMaybe++;this.debouncePredict++;this.measure++;this.frames++;__taDebug&&this.debug()},debug:function(){function a(a){a=Math.round(Math.max(0,Math.min(255,255*a)));g.fillStyle="rgb("+[a,a,a].join()+")";g.fillRect(r,y,1,20);y+=20}var c=this.data.levels.direct,
f=this.sample,e=this.signal,d=this.maybe,n=this.spectrum,m=this.histogram,h=this.data.beat,k=this,s=this.n;if(this.beat){var p=this.beat,q=1.5*p.offset;_.each(m,function(a){var c=a==p?1:0;a.offset>q&&(c=a.offset/p.offset,c=Math.max(0,1-4*Math.abs(c-Math.round(c))));a.match=c})}var t=["<strong><span"+(h.locked?' style="color: rgb(180,255,0)"':"")+">"+Math.round(10*h.bpm)/10+" BPM </span> ("+Math.round(100*h.confidence)+"%) P:"+Math.round(100*h.permanence)+" \u00b5 = "+Math.round(10*this.mean)/10+" \u03c3 = "+
Math.round(100*this.stddev)/100+"</strong> "+this.found+"f "+this.missed+"m"];_.each(m,function(a){var c=Math.round(10*k.offsetToBPM(a.offset))/10;t.push([c," bpm - ",Math.round(100*a.strength),"% P: ",Math.round(100*a.permanence)].join(""))});this.t.innerHTML=t.join("<br>");var g=this.g;g.fillStyle="#000000";g.fillRect(0,169,512,102);for(var w=0,r=2;r<s/8;++r)w=Math.max(n[r],w);w=1/w;g.beginPath();g.moveTo(0,270);for(r=2;r<s/8;++r)g.lineTo(8*(r-2),270-n[r]*w*100);g.strokeStyle="#ffffff";g.stroke();
_.each(m,function(a){var c=0.75*a.strength+0.25,d=a.active?[Math.round(255-195*a.match),Math.round(180+40*a.match),0].join():"255,10,10";g.fillStyle="rgba("+d+","+c+")";g.fillRect(8*(a.offset-2),170,1,100)});var r=this.i,y=0;a(c[1]);a(c[2]);a(c[3]);a(0);a(0);a(0);a(0);a(0);g.fillStyle="rgba(160,160,255,.85)";g.fillRect(r+1,0,2,120);h.is&&(g.fillStyle=h.missed?"rgba(255,0,0,.7)":h.maybe?"rgba(0,180,255,.9)":h.predicted?"rgba(255,200,0,1)":"rgba(60,220,0,1)",g.fillRect(this.i,120,2,40));c=Math.round(Math.max(0,
Math.min(255,255*h.was)));g.fillStyle="rgb("+c+","+c+","+c+")";g.fillRect(342,270,170,30);h.maybe&&!h.is&&(g.fillStyle="rgba(64,64,64,.75)",g.fillRect(this.i,120,2,40));f=Math.floor(255*Math.max(0,Math.min(1,1.4*f+0.5)));g.fillStyle="rgba("+Math.round(0.2*f)+","+Math.round(0.6*f)+","+f+",1)";g.fillRect(this.i,60,1,20);e=Math.floor(255*Math.max(0,Math.min(1,2*e)));g.fillStyle="rgba("+Math.round(0.2*e)+","+Math.round(e)+","+Math.round(0.5*e)+",1)";g.fillRect(this.i,80,1,20);d=h.is||h.maybe?Math.floor(255*
Math.max(0,Math.min(1,1.2*d+0.5))):0;g.fillStyle="rgba("+Math.round(d)+","+d+","+Math.round(d)+",1)";g.fillRect(this.i,100,1,20);this.i=(r+1)%512}};